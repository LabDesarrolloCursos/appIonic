...
describe('Album e2e test', () => {
  ...
  it('should load Albums', async () => {
    ...
    await element.all(by.css('ion-item'))
      .filter(async el => (await el.element(by.css('h2')).getText()) === 'Album').first().click();

    albumComponentsPage = new AlbumComponentsPage();
    await browser.wait(ec.visibilityOf(albumComponentsPage.title), 5000);
    expect(await albumComponentsPage.getTitle())
  ...
  it('should create Album', async () => {
    ...
    await browser.wait(ec.visibilityOf(albumUpdatePage.pageTitle), 1000);
    expect(await albumUpdatePage.getPageTitle())
    ...
    await browser.wait(ec.visibilityOf(albumComponentsPage.title), 1000);
    expect(await albumComponentsPage.getTitle())
      .toEqual(COMPONENT_TITLE);
    expect(await albumComponentsPage.getEntitiesNumber())
  ...
  it('should view the last Album', async () => {
    ...
    expect(await albumDetailPage.getPageTitle())
      .toEqual(SUBCOMPONENT_TITLE);
